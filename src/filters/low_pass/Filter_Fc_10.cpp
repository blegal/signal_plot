#include "Filter_Fc_10.h"

Filter_Fc_10::Filter_Fc_10() :
    Filter("Filter Fc/10")
{
...
void Filter_Fc_10::process(RawSound* _in, RawSound* _out){
    startTimer();
    ...
    if( _in->channels() == 1 ){
        ...
        for (int i = 0; i < n; i++)
        {
            ...
            for(int z=0; z<nb_taps; z++){
                yn += (float)in[i-z] * h_Filter_Fc_10[z];
            }
            out[i] = yn;
        }
    }else{
